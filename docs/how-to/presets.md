...
- Configure the React Router config on your behalf.
- Validate the resolved config.

The config returned by each preset is merged in the order they were defined. Any config directly passed to the React Router config will be merged last. This means that user config will always take precedence over any presets.
...
In our example preset, the `serverBundles` function could be overridden with a different, conflicting implementation. If we want to validate that the final resolved config contains the `serverBundles` function from our preset, we can do this with the `reactRouterConfigResolved` hook:
...
The `reactRouterConfigResolved` hook should only be used in cases where it would be an error to merge or override your preset's config.
