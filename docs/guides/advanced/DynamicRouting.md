...
Routes may define [`getChildRoutes`](../../api/ConfigurationComponents.md#getChildRoutes) and [`getComponents`](../../api/ConfigurationComponents.md#getComponents) methods. These are asynchronous and only called when needed. We call it "gradual matching". React Router will gradually match the URL and fetch only the amount of route configuration and components it needs to match the URL and render.
