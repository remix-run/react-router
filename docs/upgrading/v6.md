...
In v7 we no longer need `"react-router-dom"` as the packages have been simplified:
...
#### Linux (If you have GNU `sed` installed):
...
#### Windows:
```shellscript nonumber
find ./path/to/src \( -name "*.tsx" -o -name "*.ts" -o -name "*.js" -o -name "*.jsx" \) -type f -exec grep -l "react-router-dom" {} + | head -5
```
