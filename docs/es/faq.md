# Preguntas Frecuentes (FAQ) sobre React Router v6
Aquí se encuentran algunas preguntas que la comunidad tiene comunmente sobre React Router v6:

## ¿Qué sucedió con `withRouter`? ¡Lo necesito!

Esta pregunta surge usualmente del hecho de que utilizas componentes de clase de React, los cuales no soportan Hooks. En
React Router v6, hemos implementado completamente los Hooks y los utilizamos para compartir todo el estado interno 
del Router. Pero eso no significa que no puedas utilizar el Router. Asumiendo que actualmente puedes utilizar Hooks (estás
 en React 16.8+), solamente necesitarás un container (wrapper).
...
## ¿Por qué `<Route>` tiene una prop `element` en vez de `render` o `component`?

Mencionamos esto [en la guía de migración de v5 a v6](advanced-guides/migrating-5-to-6.md#advantages-of-route-element), 
pero es util recordarlo aquí.
