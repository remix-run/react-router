...
function Router(pattern, name, handler, callback) {
  ...
  // For now, the handler must be a React component. We could possibly
  ...
  this.displayName = componentToString(handler) + 'Router';
...
mergeInto(Router.prototype, {
  ...
  getComponentProps: function (currentPath) {
    ...
    return match.reduceRight(function (childProps, m) {
      var props = makeProps(m.router, m.params, query);

      // If we wanted to let users instantiate their own child component
      // we could do something like this:
      // props.handlerProps = childProps;
      // Then they could do:
      // this.props.handler(this.props.handlerProps)
  ...
  renderComponent: function (node) {
    if (!this._component) {
      ...
      this._component = this.handler(this.getComponentProps(urlStore.getCurrentPath()));
...
function makeProps(router, params, query) {
  var props = {
  ...
  if (router._componentProps)
    return merge(router._componentProps, props);
