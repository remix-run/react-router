...
mergeInto(Router, {
   ...
   * the given params interpolated and query appended (see Director.resolveTo). The
   * result may safely be used as the href of a link.
   */
  makeHref: function (to, params, query) {
    var href = Path.withQuery(Path.injectParams(Director.resolveTo(to), params), query);
  ...
  transitionTo: function (to, params, query) {
    URLStore.push(Director.makeHref(to, params, query));
  },
  ...
  replaceWith: function (to, params, query) {
    URLStore.replace(Director.makeHref(to, params, query));
  },
...
mergeInto(Router.prototype, {
  ...
  renderComponent: function (node) {
    invariant(
      !this._component,
      'Rendering the same router multiple times to the same page is not supported'
    );
  ...
  /**
   * Executes a transition to the given path, checking all transition hooks
   * along the way to ensure the transition is valid.
   *
   ...
   * ----------------
   *
   * Route handlers may use the willTransitionTo and willTransitionFrom static
   ...
   *
   * When transitioning away from a route, the willTransitionFrom hook is called
...
/**
 * Figures out which routers we're transitioning from and to and runs all
 * transition hooks in serial, starting with the willTransitionFrom hook on the
 * most deeply nested handler that is currently active, up the route hierarchy
 * to the common parent route, and back down the route hierarchy ending with the
 * willTransitionTo hook on the most deeply nested handler that will be active
 * if the transition completes successfully.
 *
 * After all hooks complete successfully, the currentContext of all routers
 * involved in the trasition is updated accordingly. Routes that we're
 * transitioning to keep the result of willTransitionTo as their currentContext.
 *
...
function checkTransitionHooks(lastMatches, currentMatches, transition) {
  ...
  return checkTransitionFromHooks(fromMatches, transition).then(function () {
    ...
    return checkTransitionToHooks(toMatches, transition).then(function (contexts) {
      ...
      // and handler instance from routers we're transitioning away from.
