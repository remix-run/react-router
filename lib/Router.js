...
function Router(options) {
  ...
  invariant(
    ...
    'Handler for router "' + this.name + '" must be a valid React component'
  );
...
mergeProperties(Router.prototype, {
  ...
  /**
   * Adds the given router to this router's children.
   */
  addChild: function (router) {
    this.paramNames.forEach(function (paramName) {
      invariant(
        router.paramNames.indexOf(paramName) !== -1,
        'The nested route path "' + router.path + '" is missing the "' + paramName + '" ' +
        'parameter of its parent path "' + this.path + '"'
      );
    }, this);
  ...
  match: function (path) {
    ...
    for (var i = 0, length = childRouters.length; i < length; ++i) {
      matches = childRouters[i].match(path);
      ...
      if (matches) {
        ...
        matches.unshift(makeMatch(this, params));
    ...
    var params = Path.extractParams(this.path, path);
...
mergeProperties(Router, {
  ...
  fromRouteComponent: function (routeComponent) {
    ...
    React.Children.forEach(routeComponent.props.children, function (child) {
      router.addChild(Router.fromRouteComponent(child));
    });
...
function computeChanges(router, transition) {
  ...
  var currentMatches = router._currentMatches;
  ...
  return checkTransitionFromHooks(fromMatches, transition).then(function () {
    ...
    return checkTransitionToHooks(toMatches, transition).then(function () {
      ...
      return function () {
        invariant(
          router._currentMatches === currentMatches,
          'Changes for router ' + router + ' are out of sync with its current state'
        );

        router._currentMatches = nextMatches;
        router._currentPath = transition.path;

        var props = computeProps(nextMatches, Path.extractQuery(transition.path));

        if (router._component)
          router._component.setProps(props);
      };
    });
  });
}
...
function computeProps(matches, query) {
  ...
  reversedArray(matches).forEach(function (match) {
    ...
    match.handlerInstance = router.handler(props);
