...
function Router(routeComponent) {
  ...
  this.route = makeRoute(routeComponent);
  this.state = {};
  this.components = [];
}
...
function makeRoute(routeComponent, _parentRoute) {
  invariant(
    React.isValidComponent(routeComponent) && routeComponent.type === Route.type,
    'The Router may only contain <Route> components'
  );

  var props = routeComponent.props;
  var path = Path.normalize(props.path || props.name || '/');
  var route = {
    name: props.name,
    path: path,
    handler: props.handler,
    paramNames: Path.extractParamNames(path),
    staticProps: Route.getUnreservedProps(props)
  };

  // Make sure the route has a valid handler.
  invariant(
    React.isValidComponent(route.handler),
    'The handler for route "' + route.name + '" must be a valid React component'
  );
  ...
  if (_parentRoute) {
    _parentRoute.paramNames.forEach(function (paramName) {
      invariant(
        ...
        'The nested route path "' + route.path + '" is missing the "' + paramName + '" ' +
  ...
  if (route.name) {
    invariant(
      !_namedRoutes[route.name],
      'You cannot use the name "' + route.name + '" for more than one route'
    );

    _namedRoutes[route.name] = route;
  }

  // If the route has children, store them in childRoutes.
  if (props.children) {
    var childRoutes = route.childRoutes = [];

    React.Children.forEach(props.children, function (child) {
      childRoutes.push(makeRoute(child, route));
    });
  }
