...
var Routes = React.createClass({
  ...
  componentWillMount: function () {
    ...
    if (urlStore.isSetup()) {
      var currentLocation = urlStore.getLocation();

      warning(
        currentLocation === location,
        'Another <Routes> component is already using ' + currentLocation + ' location, so you cannot use ' + location + ' location on the same page'
      );
    } else {
  ...
  updateActive: function (currentPath) {
    ...
    warning(!(currentPath && !routes.length), 'No routes matched path "' + currentPath + '"');
