...
function setup(location) {
  invariant(!isSetup(), 'You cannot setup the URL store twice');

  var changeEvent = CHANGE_EVENTS[location];

  invariant(changeEvent, 'The URL store location "' + location + '" is not valid');
...
function teardown() {
  ...
  window.removeEventListener(changeEvent, handleStateChange, false);
