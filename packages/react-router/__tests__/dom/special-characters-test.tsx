...
describe("special character tests", () => {
  ...
  describe("when matching as param values", () => {
    ...
    it("handles encoded slashes in ancestor splat route segments", async () => {
      let ctx = render(
        <BrowserRouter window={getWindow("/parent/child/slash-%2F-sign")}>
          <App />
        </BrowserRouter>,
      );
      ...
      expect(getHtml(ctx.container)).toMatchInlineSnapshot(`
       "<div>
         <a
           data-discover="true"
           href="/parent/child/slash-%2F-sign/grandchild"
         >
           Link to grandchild
         </a>
       </div>"
