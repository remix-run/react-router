...
function createRouter(init) {
	...
	async function startNavigation(historyAction, location, opts) {
		...
		let matches = opts?.initialHydration && state.matches && state.matches.length > 0 && !initialMatchesIsFOW ? state.matches : dataRouteMatcher.match(location, false);
		let flushSync = (opts && opts.flushSync) === true;
...
function getMatchesToLoad(request, scopedContext, mapRouteProperties, manifest, history, state, matches, submission, location, lazyRoutePropertiesToSkip, initialHydration, isRevalidationRequired, cancelledFetcherLoads, fetchersQueuedForDeletion, fetchLoadMatches, fetchRedirectIds, hasPatchRoutesOnNavigation, dataRouteMatcher, pendingActionResult, callSiteDefaultShouldRevalidate) {
