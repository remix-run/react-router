...
import { DataRouterContext, DataRouterNavigationContext, DataRouterStateContext, FetchersContext, NavigationContext, RouteContext, ViewTransitionContext } from "../context.js";
import { useBlocker, useCurrentRouteId, useHref, useLocation, useMatches, useNavigate, useNavigation, useResolvedPath } from "../hooks.js";
...
const NavLink = React$1.forwardRef(function NavLinkWithRef({ "aria-current": ariaCurrentProp = "page", caseSensitive = false, className: classNameProp = "", end = false, style: styleProp, to, viewTransition, children, ...rest }, ref) {
	...
	let location = useLocation();
	let routerNavigation = React$1.useContext(DataRouterNavigationContext);
...
function useFetcher({ key } = {}) {
	...
	let fetchersContext = React$1.useContext(FetchersContext);
	let routeId = useCurrentRouteId("useFetcher");
	invariant(fetchersContext, `useFetcher must be used inside a FetchersContext`);
...
function useFetchers() {
	...
	return React$1.useMemo(() => Array.from(fetchers.entries()).map(([key, fetcher]) => ({
		...fetcher,
		key
	})), [fetchers]);
}
