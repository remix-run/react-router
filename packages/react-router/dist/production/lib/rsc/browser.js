...
import { createSingleFetchRequestInit, getSingleFetchDataStrategyImpl, singleFetchUrl, stripIndexParam } from "../dom/ssr/single-fetch.js";
...
function getRSCSingleFetchDataStrategy(getRouter, ssr, createFromReadableStream, fetchImplementation) {
	...
	}, getFetchAndDecodeViaRSC(createFromReadableStream, fetchImplementation, () => getRouter().future.unstable_traverseCache === true), ssr, (match) => {
		let M = match;
...
function getFetchAndDecodeViaRSC(createFromReadableStream, fetchImplementation, getUnstableTraverseCache) {
	return async (args, targetRoutes) => {
		...
		let res = await fetchImplementation(new Request(url, await createSingleFetchRequestInit(args, getUnstableTraverseCache())));
