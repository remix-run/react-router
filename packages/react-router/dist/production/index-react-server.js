...
function matchRouteBranch(branch, pathname, allowPartial = false) {
	...
	for (let i = 0; i < routesMeta.length; ++i) {
		...
		matches.push({
			...
			pathnameBase: normalizePathname$1(joinPaths([matchedPathname, match.pathnameBase])),
			route
		});
...
const normalizePathname$1 = (pathname) => removeTrailingSlash(pathname).replace(/^\/*/, "/");
...
var RoutePatternDataRouteMatcher = class {
	...
	update(routes) {
		...
		let matcher = createPathnameMultiMatcher({ ignoreCase: true });
		let partialMatcher = createPathnameMultiMatcher({ ignoreCase: true });
...
function flattenRoutes(routes, branches = [], parentsMeta = [], parentPath = "") {
	routes.forEach((route, index) => {
		...
		if (meta.relativePath.startsWith("/")) {
			...
			invariant$1(parentPathMatch != null, `Absolute route path "${meta.relativePath}" nested under path "${parentPath}" is not valid. An absolute child route path must start with the combined path of all its parent routes.`);
...
function matchRoutePatternBranch(branch, pathname, allowPartial, metaIndex, matchedPathname, matchedParams) {
	...
	for (let relativePath of explodeOptionalSegments(meta.relativePath)) {
		...
		let routeMatch = {
			...
			pathnameBase: normalizePathname$1(joinPaths([matchedPathname, pathMatch.pathnameBase])),
			route: meta.route
		};
