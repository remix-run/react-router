...
function flattenRoutes$1(routes, branches = [], parentsMeta = [], parentPath = "", _hasParentOptionalSegments = false) {
	let flattenRoute = (route, index, hasParentOptionalSegments = _hasParentOptionalSegments, relativePath) => {
		...
		if (route.children && route.children.length > 0) {
			...
			flattenRoutes$1(route.children, branches, routesMeta, path, hasParentOptionalSegments);
		}
...
var RoutePatternDataRouteMatcher = class {
	...
	match(locationArg, allowPartial = false) {
		...
		invariant$1(this.#state, "Route pattern routes must be initialized before matching.");
...
function validateRoutePatternSplat(routePath, routePattern) {
	invariant$1(getRoutePatternSplatIndexes(routePattern).length <= 1 && isTerminalRoutePatternSplat(routePattern), `Route path "${routePath}" is not supported with \`future.unstable_routePatternMatching\` because React Router only supports a single splat at the end of a route path.`);
}
...
function flattenRoutes(routes, branches = [], parentsMeta = [], parentPath = "") {
	routes.forEach((route, index) => {
		invariant$1(route.caseSensitive !== true, "`caseSensitive` routes are not supported with `future.unstable_routePatternMatching`.");
		let meta = {
...
function prioritizeValidatedMatches(matches) {
	...
	for (let match of matches) if (match.data.routesMeta.some((meta) => meta.route.unstable_validateParams != null && Object.keys(meta.route.unstable_validateParams).length > 0)) validatedMatches.push(match);
...
function createDataRouteMatcher(future, basename) {
	return future.unstable_routePatternMatching ? new RoutePatternDataRouteMatcher(basename) : new V6RegExMatcher(basename);
}
/**
...
function createRSCDataRouteMatcher(routes, basename) {
...
async function matchRSCServerRequest({ allowedActionOrigins, createTemporaryReferenceSet, basename, decodeReply, requestContext, routeDiscovery, loadServerAction, decodeAction, decodeFormState, clientVersion, onError, request, routes, generateResponse }) {
	...
	let matches = createRSCDataRouteMatcher(routes, basename).match(url.pathname);
...
async function generateManifestResponse(routes, basename, request, generateResponse, temporaryReferences, routeDiscovery, clientVersion) {
	...
	let matchedRoutes = pathnames.flatMap((pathname) => {
		let pathnameMatches = dataRouteMatcher.match(pathname);
	...
	let payload = {
		type: "manifest",
		patches: Promise.all([...matchedRoutes.map((route) => getManifestRoute(route)), getAdditionalRoutePatches(pathnames, dataRouteMatcher, Array.from(routeIds))]).then((r) => r.flat(1))
	};
	return generateResponse({
		statusCode: 200,
...
async function getRenderPayload(baseRenderPayload, routes, basename, routeIdsToLoad, isDataRequest, staticContext, routeDiscovery) {
	...
	if (routeDiscovery?.mode === "initial" && !isDataRequest) patches = getAllRoutePatches(routes, basename).then((patches) => patches.filter((patch) => !staticContext.matches.some((m) => m.route.id === patch.id)));
		...
		let dataRouteMatcher = createRSCDataRouteMatcher(routes, basename);
		patches = getAdditionalRoutePatches(getPathsWithAncestors([staticContext.location.pathname]), dataRouteMatcher, staticContext.matches.map((m) => m.route.id));
	}
	return {
		...baseRenderPayload,
...
async function getAdditionalRoutePatches(pathnames, dataRouteMatcher, matchedRouteIds) {
