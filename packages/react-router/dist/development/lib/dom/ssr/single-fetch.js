...
function getTurboStreamSingleFetchDataStrategy(getRouter, manifest, routeModules, ssr) {
	...
	}, (args, targetRoutes) => fetchAndDecodeViaTurboStream(args, targetRoutes, getRouter().future.unstable_traverseCache === true), ssr);
...
async function fetchAndDecodeViaTurboStream(args, targetRoutes, unstableTraverseCache = false) {
	let { request } = args;
	...
	let res = await fetch(url, await createSingleFetchRequestInit(args, unstableTraverseCache));
...
async function createSingleFetchRequestInit(args, unstableTraverseCache = false) {
	let init = await createRequestInit(args.request);
	if (unstableTraverseCache) init.cache = args.fetcherKey == null && args.navigationType === "POP" ? "force-cache" : "default";
	return init;
}
...
//#endregion
export { NO_BODY_STATUS_CODES, SingleFetchRedirectSymbol, StreamTransfer, createSingleFetchRequestInit, decodeViaTurboStream, getSingleFetchDataStrategyImpl, getTurboStreamSingleFetchDataStrategy, singleFetchUrl, stripIndexParam };
