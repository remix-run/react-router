...
import { IDLE_BLOCKER, IDLE_FETCHER, IDLE_NAVIGATION, createDataRouteMatcher } from "../router/router.js";
...
function createStaticRouter(routes, context, opts = {}) {
	...
	return {
		...
		match(locationArg) {
			let routeMatches;
			if (matchRoutes) routeMatches = matchRoutes(locationArg);
			else {
				if (!dataRouteMatcher) {
					dataRouteMatcher = createDataRouteMatcher(future, context.basename || "/");
