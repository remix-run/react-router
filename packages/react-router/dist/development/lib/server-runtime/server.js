...
import { createDataRouteMatcher, createStaticHandler, getStaticContextFromError, isMutationMethod, isRedirectResponse, isResponse } from "../router/router.js";
...
function derive(build, mode) {
	...
	let requestHandler = async (request, initialContext) => {
		...
		let dataRouteMatcher = createDataRouteMatcher(build.future, build.basename ?? "/");
		dataRouteMatcher.update(staticHandler.dataRoutes);
		if (build.routeDiscovery.mode === "lazy" && requestUrl.pathname === manifestUrl) try {
			return await handleManifestRequest(build, dataRouteMatcher, requestUrl);
		} catch (e) {
			handleError(e);
			return new Response("Unknown Server Error", { status: 500 });
		}
		let matches = matchServerRoutes(build.routes, dataRouteMatcher, normalizedPathname);
...
async function handleManifestRequest(build, dataRouteMatcher, url) {
	...
	if (url.searchParams.has("paths")) {
		...
		for (let path of paths) {
			...
			let matches = matchServerRoutes(build.routes, dataRouteMatcher, path);
