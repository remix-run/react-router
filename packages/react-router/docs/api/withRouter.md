...
By default `withRouter` does not subscribe to location changes like React Redux's `connect` does for state changes. Instead, re-renders after location changes propagate out from the `<Router>` component. This means that `withRouter` does _not_ re-render on route transitions unless its parent component re-renders. If you are using `withRouter` to prevent updates from being blocked by `shouldComponentUpdate`, it is important to use the `updateOnLocationChange` configuration option of `withRouter`. For example, when using Redux:

```js
// This gets around shouldComponentUpdate
connect(...)(withRouter(MyComponent, {updateOnLocationChange: true}))
// or
compose(
  connect(...)
  Component => withRouter(Component, {updateOnLocationChange: true})
)(MyComponent)
