...
/**
  Returns a object with the current search parameters and a function to update them. Setting the search params causes a navigation. This hook is a more advanced version of {@link useSearchParams} that allows you to define mandatory and optional parameters, default values, and more.

  ```tsx
  import { searchParams, getParams, getParam, clearParams updateParams, onChange  } from "react-router";
...
>({
  ...
  const subscriptionsRef = React.useRef<Record<string, Array<() => unknown>>>({}); 
  const previousParamsRef = React.useRef<Record<string, unknown>>({})

  const TOTAL_PARAMS_PAGE: MergeParams<M, O> = React.useMemo(() => {
    return { ...mandatory, ...optional };
  }, [mandatory, optional]);

  const PARAM_ORDER = React.useMemo(() => {
  ...
  const ARRAY_KEYS = React.useMemo(() => {
   ...
   */
    ...
    const CURRENT_PARAMS_URL: Record<string, unknown> = React.useMemo(() => {
   ...
   */
    const onChange = React.useCallback( (paramName: keys, callbacks: Array<() => void>) => {
