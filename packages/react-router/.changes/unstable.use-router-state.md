...
- Accepts an optional `routeId` to scope matching (mirroring `unstable_useRoute`) — `active`/`pending` are `null` when no match in the corresponding location has that id
