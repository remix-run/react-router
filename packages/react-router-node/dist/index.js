...
function monitorWritableError(writable) {
	...
	let writableError;
	let pendingRejections = /* @__PURE__ */ new Set();
	function cleanup() {
	...
	function reject(error) {
		...
		for (let rejectPending of rejections) rejectPending(error);
	}
	...
	return {
		cleanup,
		race(promise) {
			if (writableError) return Promise.race([promise, Promise.reject(writableError)]);
			let rejectPending;
			let pendingPromise = new Promise((_, reject) => {
