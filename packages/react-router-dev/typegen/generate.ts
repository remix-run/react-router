...
export function generateRoutes(ctx: Context): Array<VirtualFile> {
  ...
  // `Pages` and `RouteFiles` are made of string literals, but `RouteModules` is
  // made of `typeof import(...)` of every route module. Since `+routes.ts`
  // augments `react-router`, TypeScript adds an edge from every file that
  // imports `react-router` back to `+routes.ts`, so those route module imports
  // close a cycle: `+routes.ts` -> route modules -> anything importing
  // `react-router` -> `+routes.ts`. `tsc` invalidates a strongly connected
  // component as a unit, so a public shape change to any file in that cycle
  // re-checks all of it. Only pay for that when the app opts into precise
  // `unstable_useRoute` types.
  const registerRouteModules = ctx.config.future.unstable_routeModuleTypes;
  ...
  const routesTs: VirtualFile = {
    ...
    content:
      [
        ...
        ...(registerRouteModules ? ["    routeModules: RouteModules"] : []),
        "  }",
        "}",
      ].join("\n") +
      "\n\n" +
