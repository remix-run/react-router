...
const resolveRouteFileCode = async (
  ctx: ReactRouterPluginContext,
  input: string | { routeFile: string; viteChildCompiler: Vite.ViteDevServer }
): Promise<string> =>
  typeof input === "string"
    ? input
    : await compileRouteFile(input.viteChildCompiler, ctx, input.routeFile);
