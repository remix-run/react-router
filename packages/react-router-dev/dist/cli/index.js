 ...
 * @license MIT
 */
import { g as preloadVite, h as getVite, l as loadConfig, n as watch, t as run$1 } from "../typegen-DphYyWly.js";
...
async function build(root, options = {}) {
	...
	let { build } = await import("../build-B-6XUuob.js");
...
async function dev(root, options = {}) {
	let { dev } = await import("../dev-BN26xeNI.js");
	if (options.profile) await start();
...
async function generateEntry(entry, rootDirectory, flags = {}) {
	...
	if (isRsc) {
		...
		let defaultEntryServer = path$1.resolve(defaultsDirectory, `entry.server.tsx`);
