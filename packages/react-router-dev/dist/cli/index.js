 ...
 * @license MIT
 */
import { _ as getVite, l as hasNodeDependency, n as watch, t as run$1, u as loadConfig, v as preloadVite } from "../typegen-hQHDLC1I.js";
...
*/
function resolveRestartSpawn(argv, extraOptions, env) {
...
function restartWithMergedOptions(nodeOptions) {
	...
	const { command, args, env } = resolveRestartSpawn(process$1.argv, nodeOptions, process$1.env);
...
async function build(root, options = {}) {
	...
	let { build } = await import("../build-BmJq8JCn.js");
...
async function dev(root, options = {}) {
	if (developmentConditionEnabled) {
		let { dev } = await import("../dev-B6M7hiGK.js");
		if (options.profile) await start();
