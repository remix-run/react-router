 ...
 * @license MIT
 */
import { _ as getVite, a as t, c as createConfigLoader, d as resolveEntryFiles, f as resolveRSCEntryFiles, g as getUserBuildRollupOptions, h as defineOptimizeDepsCompilerOptions, i as parse$1, l as hasNodeDependency, m as defineCompilerOptions, n as watch, o as traverse, p as ssrExternals, r as generate, s as configRouteToBranchRoute, v as preloadVite } from "./typegen-hQHDLC1I.js";
import { u as invariant } from "./routes-Cyup1Gnm.js";
...
async function fromNodeRequest(nodeReq, nodeRes) {
	...
	return createRequest(nodeReq, nodeRes, { protocol: getForwardedProtocol(nodeReq) });
}
function getForwardedProtocol(nodeReq) {
	let forwardedProto = nodeReq.headers["x-forwarded-proto"];
	let proto = (Array.isArray(forwardedProto) ? forwardedProto[0] : forwardedProto)?.split(",")[0].trim().toLowerCase();
	if (proto?.endsWith(":")) proto = proto.slice(0, -1);
	return proto === "http" || proto === "https" ? `${proto}:` : void 0;
}
//#endregion
...
function getRouteChunkCode(code, chunkName, cache, cacheKey, generateOptions = {}) {
	if (chunkName === mainChunkName) return omitChunkedExports(code, routeChunkExportNames, generateOptions, cache, cacheKey);
	...
	return [
		...
		},
		{
			...
			async transform(code, id, options) {
				...
				return {
					code: `export { ${sourceExports.filter((exportName) => {
...
async function getRouteChunkIfEnabled(cache, ctx, id, chunkName, input) {
	...
	return getRouteChunkCode(await resolveRouteFileCode(ctx, input), chunkName, cache, normalizeRelativeFilePath(id, ctx.reactRouterConfig) + (typeof input === "string" ? "" : "?read"), {
		sourceMaps: true,
		sourceFileName: path$2.basename(id.split("?")[0])
	});
}
...
function generateRouteModule(code, map, filename) {
	...
	return {
		ast,
		generate: () => generate(ast, generatorOptions)
	};
}
function prependUnmappedCode(prefix, generated) {
	if (generated.map) generated.map.mappings = ";".repeat(prefix.match(/\n/g)?.length ?? 0) + generated.map.mappings;
	return {
		code: prefix + generated.code,
		map: generated.map
	};
}
function appendUnmappedCode(generated, code) {
	if (!generated.code.endsWith("\n") && !code.startsWith("\n")) code = "\n" + code;
	if (generated.map) generated.map.mappings += ";A".repeat(code.match(/\n/g)?.length ?? 0);
	generated.code += code;
}
...
${result}`;
	...
	function createServerRouteModule(code, map, filename) {
		const { ast, generate } = generateRouteModule(code, map, filename);
		removeExports(ast, CLIENT_ROUTE_EXPORTS);
		return generate();
	}
	async function createClientRouteModuleChunk(id, code, chunk, routeId, isRootRouteModule, isDevMode, map, filename) {
		let routeChunks = detectRouteChunks(cache, id, code, isRootRouteModule);
		const { ast, generate } = generateRouteModule(code, map, filename);
		...
		if (chunk === "shared") {
			if (isRootRouteModule && !staticExports.includes("ErrorBoundary") && !staticExports.includes("ServerErrorBoundary")) appendUnmappedCode(result, `
import { createElement as __rr_createElement } from "react";
import { UNSAFE_RSCDefaultRootErrorBoundary } from "react-router";
export function ErrorBoundary() {
  return __rr_createElement(UNSAFE_RSCDefaultRootErrorBoundary, { hasRootLayout: ${staticExports.includes("Layout") || staticExports.includes("ServerLayout")} });\n}\n`);
		...
		if (isDevMode) appendUnmappedCode(result, `export function ReactRouterHMRMeta___() {return null;};\nObject.assign(ReactRouterHMRMeta___, {
	...
	return {
		...
		async transform(_code, id) {
			...
			if (clientRouteModuleType) return await createClientRouteModuleChunk(id, code, clientRouteModuleType, routeId, isRootRouteModule(filename), this.environment.mode === "dev", transformed.map, filename);
			if (isServerRouteModule) return createServerRouteModule(code, transformed.map, filename);
	...
	return {
		...
		transform: {
			...
			async handler(code, id) {
				...
				return {
					code: createClientRouteModuleForOptimizeDepsScan((await transformToJs(code, filename)).code).code,
					map: null,
					moduleType: "js"
				};
			}
