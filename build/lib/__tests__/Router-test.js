  ...
  describe('query params', function () {
    it('renders with query params', function (done) {
      ...
      Router.run(routes, '/?foo=bar', function (Handler, state) {
        var html = ReactDOMServer.renderToString(React.createElement(Handler, { foo: state.query.foo }));
  ...
  describe('willTransitionFrom', function () {
    ...
    it('should be called when Handler is rendered multiple times on same route', function (done) {
      ...
      Router.run(routes, location, function (Handler, state) {
        ...
        ReactDOM.render(React.createElement(Handler, { data: { FooBar: 1 } }), div);

        ReactDOM.render(React.createElement(Handler, { data: { FooBar: 1 } }), div, function () {
          setTimeout(function () {
            steps.shift()();
          }, 1);
        });
...
describe('Router.run', function () {
  ...
  it('matches a root route', function (done) {
    ...
    Router.run(routes, '/', function (Handler, state) {
      var html = ReactDOMServer.renderToString(React.createElement(Handler, { foo: 'bar' }));
      expect(html).toMatch(/bar/);
      done();
    });
  });
